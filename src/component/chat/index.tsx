...
export default function Chat({ users }: any) {
  ...
  const handleRollDice = (text: string) => {
    return new Promise((resolve, reject) => {
      ...
      const regexD12 =
        /\b(2d12|\d+d6)\s*\+\s*\d+d6\b|\b\d+d6\s*\+\s*(2d12|\d+d6)\b/g;
      ...
      if (!isValid) {
        ...
        resultText += `F: ${F}\n`;
        resultText += `D: ${D}\n`;
        if (d12Results.includes(12)) resultText += `Gandalf!`;
        if (d12Results.includes(11)) resultText += `Sauron!`;
  ...
  return (
    <>
      ...
      <div
        className={`${styles.chatContainer} ${isVisible ? styles.visible : ""}`}
      >
        ...
        <div className={styles.fundoHistorico} ref={containerRef}>
          {msg &&
            msg.map((cada: Message) => (
              <div
                key={cada.id}
                className={`${
                  cada.isMine ? styles.alignRight : styles.alignLeft
                }`}
              >
                {cada.isMine ? (
        ...
        {rollChoiceOn && (
          <RollChoice
            rollTestOn={rollTestOn}
            setRollTestOn={setRollTestOn}
            choice={choice}
            setChoice={setChoice}
            setTest={setTest}
            setRolling={setRolling}
